...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  ...
  function account(url) {
    ...
    if (url.indexOf('token=') == -1) {
      var token = 'zcy3rjng';
      if (token != '') url = Lampa.Utils.addUrlComponent(url, 'token=zcy3rjng');
    }
  ...
  function startPlugin() {
    ...
    var manifst = {
      ...
      onContextLauch: function onContextLauch(object) {
        resetTemplates();
        Lampa.Component.add('MaxUA', component);
	...
	addSourceSearch('MaxUA', 'spider');
	addSourceSearch('MaxUA - Anime', 'spider/anime');
    ...
    Lampa.Component.add('MaxUA', component); //то же самое
    resetTemplates();
