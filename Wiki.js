(function () {
    ...
    function WikiSmartPlugin() {
        ...
        this.startSearch = function (movie) {
            ...
            var titleRU = (movie.title || movie.name || '').replace(/[^\w\sа-яё]/gi, '');
            var isTV = !!(movie.first_air_date || movie.number_of_seasons);
            
            var results = [];
            var p1 = $.ajax({ url: 'https://ru.wikipedia.org/w/api.php', data: { action: 'query', list: 'search', srsearch: titleRU + ' ' + year + (isTV ? ' сериал' : ' фильм'), srlimit: 3, format: 'json', origin: '*' }, dataType: 'json' });
            var p2 = $.ajax({ url: 'https://uk.wikipedia.org/w/api.php', data: { action: 'query', list: 'search', srsearch: titleUA + ' ' + year + (isTV ? ' серіал' : ' фільм'), srlimit: 3, format: 'json', origin: '*' }, dataType: 'json' });
            ...
            $.when(p1, p2, p3).done(function (r1, r2, r3) {
                if (r1[0].query && r1[0].query.search) {
                    r1[0].query.search.forEach(function(i) {
                        results.push({ title: i.title, lang: '🇷🇺', url: 'https://ru.m.wikipedia.org/wiki/' + encodeURIComponent(i.title) });
                    });
                }
