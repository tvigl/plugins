(function () {
    ...
    function WikiSmartPlugin() {
        ...
        var ICON_IMDB = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" fill="#f5c518"><path d="M400 32H48C21.5 32 0 53.5 0 80v352c0 26.5 21.5 48 48 48h352c26.5 0 48-21.5 48-48V80c0-26.5-21.5-48-48-48zM122.3 346.4c0 10.2-8.2 18.4-18.4 18.4H83.4c-10.2 0-18.4-8.2-18.4-18.4V165.6c0-10.2 8.2-18.4 18.4-18.4h20.5c10.2 0 18.4 8.2 18.4 18.4v180.8zm152.1 0c0 10.2-8.2 18.4-18.4 18.4h-20.5c-10.2 0-18.4-8.2-18.4-18.4V165.6c0-10.2 8.2-18.4 18.4-18.4h20.5c10.2 0 18.4 8.2 18.4 18.4v180.8zm110.3-63.9c0 19.1-15.5 34.6-34.6 34.6h-20.5c-10.2 0-18.4-8.2-18.4-18.4V165.6c0-10.2 8.2-18.4 18.4-18.4h20.5c19.1 0 34.6 15.5 34.6 34.6v82.3zM188.7 147.2h20.5c10.2 0 18.4 8.2 18.4 18.4v180.8c0 10.2-8.2 18.4-18.4 18.4h-20.5c-10.2 0-18.4-8.2-18.4-18.4V165.6c0-10.2 8.2-18.4 18.4-18.4z"/></svg>';
        var isOpened = false;
        ...
        this.startSearch = function (movie) {
            ...
            var loader = $('<div class="wiki-loader" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 3000; display: flex; align-items: center; justify-content: center;"><div style="color: #fff; font-size: 1.5em; background: rgba(0,0,0,0.8); padding: 20px; border-radius: 10px; border: 1px solid #fff;">Поиск Wiki & IMDb...</div></div>');
            $('body').append(loader);
            ...
            // Добавляем IMDb в начало списка
            if (movie.imdb_id) {
                results.push({
                    title: (movie.title || movie.name) + (year ? ' (' + year + ')' : '') + (movie.vote_average ? ' ★ ' + movie.vote_average : ''),
                    lang: 'IMDb',
            ...
            // Добавляем поиск на IMDb для разных языков
            var imdbQueries = [
                { t: movie.original_title || movie.original_name, n: 'IMDb (Original)' },
                { t: movie.title || movie.name, n: 'IMDb (RU/UA)' }
            ];
            
            imdbQueries.forEach(function(item) {
                if (item.t) {
                    var isDuplicate = movie.imdb_id && results.some(function(r) { return r.isIMDb && r.title.indexOf(item.t) !== -1; });
                    if (!isDuplicate) {
                        results.push({
                            title: item.n + ': ' + item.t,
                            lang: 'IMDb',
                            url: 'https://www.imdb.com/find?q=' + encodeURIComponent(item.t),
                            score: 9000,
                            isIMDb: true,
                            icon: ICON_IMDB,
                            langPriority: 0
        ...
        this.showMenu = function(items, prev_controller) {
            var _this = this;
            ...
            items.slice(0, 15).forEach(function(item) {
                var icon = item.icon ? '<div class="wiki-item__icon" style="width: 1.4em; height: 1.4em; display: flex; align-items: center; justify-content: center;">' + item.icon + '</div>' : 
                ...
                el.on('hover:enter click', function() {
                    menu.remove();
                    _this.openIframe(item.url, item.title, prev_controller);
                });
        ...
        this.openIframe = function (url, title, prev_controller) {
            ...
            // Wikipedia specific
            if (cleanUrl.indexOf('wikipedia.org') !== -1) {
            ...
            // IMDb specific - IMDb usually blocks iframes. 
            // If we are on a platform that supports opening URLs, we might want to use that.
            if (cleanUrl.indexOf('imdb.com') !== -1) {
                if (window.Lampa && Lampa.Platform && Lampa.Platform.is('webos', 'tizen', 'android')) {
                    // On TVs, try to open in browser if it's IMDb
                    // But for now, let's try the iframe approach first to see if it works on some platforms
                }
            }
            ...
            var viewer = $('<div class="wiki-viewer-container">' +
                                ...
                                (cleanUrl.indexOf('imdb.com') !== -1 ? '<div class="wiki-external-btn selector" style="position: absolute; top: 10px; right: 60px; width: 45px; height: 45px; background: #f5c518; color: #000; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; z-index: 2006; font-size: 20px; font-weight: bold; border: 2px solid #000;">IMDb</div>' : '') +
            ...
            viewer.find('.wiki-external-btn').on('hover:enter click', function() {
                if (window.Lampa && Lampa.Platform && typeof Lampa.Platform.openURL === 'function') {
                    Lampa.Platform.openURL(cleanUrl);
                } else if (window.Lampa && Lampa.Utils && typeof Lampa.Utils.openURL === 'function') {
                    Lampa.Utils.openURL(cleanUrl);
                } else {
